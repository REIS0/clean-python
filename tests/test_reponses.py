...
GENERIC_REPONSE_MESSAGE = "This is a response"
...
def test_response_failure_is_false():
    response = ResponseFailure(
        GENERIC_REPONSE_TYPE, GENERIC_REPONSE_MESSAGE
    )

    assert bool(response) is False


def test_response_succcess_has_type_and_value():
    response = ResponseSuccess(SUCCESS_VALUE)
...
def test_response_failure_has_type_and_message():
    ...
    assert response.message == GENERIC_REPONSE_MESSAGE
    assert response.value == {
        "type": GENERIC_REPONSE_TYPE,
        "message": GENERIC_REPONSE_MESSAGE,
    }


def test_response_failure_initialisation_with_exception():
    response = ResponseFailure(
        GENERIC_REPONSE_TYPE, Exception("Just an error message")
    )
